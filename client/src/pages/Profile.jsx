...
export default function Profile () {
  ...
  const handleFileUpload = (file) => {
    ...
    const fileName = new Date().getTime() + file.name; // for unique name.
    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (<span className='text-red-700'>Error uploading image</span>) : fileUploadPercentage > 0 && fileUploadPercentage < 100 ? (
            <span className='text-slate-700'>{`uploading ${fileUploadPercentage}%`}</span>) : fileUploadPercentage === 100 ? (
